...
function escape_qs(param, opt){
    ...
    for (var i in param)
    {
        ...
        else if (Array.isArray(val))
        {
            if (!val.length)
                continue;
            qs += val.map(val=>key+'='+uri_comp_val(val)).join('&');
        }
        else
...
html5.Player.prototype.once = function(id, cb){
    ...
    if (s && !this.view_cbs.find(c=>(c.id==s && c.cb==cb)))
...
html5.Player.prototype.off = function(id, cb){
    ...
    if (s && (e=this.view_cbs.find(c=>(c.id==s && c.cb==cb))))
...
html5.Player.prototype.monitor_playback = function(view, dur){
    ...
    this.view_cbs = this.view_cbs.filter(s=>s!='del');
};

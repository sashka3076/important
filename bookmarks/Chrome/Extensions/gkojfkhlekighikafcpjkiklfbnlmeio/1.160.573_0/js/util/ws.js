...
class WS extends events.EventEmitter {
    ...
    zjson(data, ttl){
        return this.send(conv.JSON_stringify(data, this.zjson_opt)); }
    _check_status(){
    ...
    _on_message(event){
        ...
        if (msg instanceof ArrayBuffer)
            msg = Buffer.from(Buffer.from(msg)); 
        if (zerr.is.debug())
        {
            zerr.debug(typeof msg=='string'
                ? `${this}< str: ${string.trunc(msg, debug_str_len)}`
            ...
            if (typeof msg=='string')
            {
                if (this._events.zjson)
                {
                    ...
                    if (this.zc && this.time_parse)
                    {
                        ...
                        parsed = conv.JSON_parse_unsafe(msg, this.zjson_opt);
...
class Server {
    ...
    broadcast_zjson(data){
        if (this.connections.size)
            this.broadcast(conv.JSON_stringify(data, zjson_opt));
    }
    close(code, reason){
...
class Mux {
    ...
    open_ack(vfd, opt={}){
        ...
        const zfinish = (end_write, wait_rmt)=>etask(function*_zfinish(){
            ...
            stream.zfin = this;
            zerr.info(`${_this.ws}:vfd:${vfd}:zfin:${end_write}:${wait_rmt}`);
